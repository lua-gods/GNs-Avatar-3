local nouns = {"apple", "book", "car", "dog", "elephant", "flower", "guitar", "house", "island", "jungle", "kite", "lamp", "mountain", "notebook", "ocean", "pencil", "queen", "river", "star", "tree", "umbrella", "village", "window", "yacht", "zebra"}
...
events.TICK:register(function()
	...
	if not host:isHost() then if player:getVelocity().xz:length() > 0.1 then alive = 60 end end
end)

models.player.Newspaper:setVisible(true)
animations.player.newspaperInit:play()
function pings.idle(state) setIdle(state) end
